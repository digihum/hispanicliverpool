...
(function() {
  require(["jquery", "underscore", "backbone", "bootstrap", "backbone.paginator", "backbone-relational"], function($, _, Backbone, Bootstrap, PageableCollection) {
    var Address, Addresses, Countries, Country, FlatPerson, GeoCode, MapView, Occupation, Occupations, PaginatedPeople, People, PeopleListView, PeopleSearch, PeopleSearchResultsView, Person, PersonSingleView, Place, Places, Relationship, Relationships, Router, SearchFormView, app, htmlEncode, router;
    ...
    PeopleListView = Backbone.View.extend({
      el: "#page",
      initialize: function() {
        this.listenTo(app.paginatedPeople, "sync", this.render);
        return this.$el.html($("#view-loading").html());
      },
      breadcrumb: function(collection) {
        ...
        breadcrumb_template = _.template($("#breadcrumb-template").html(), {
          breadcrumbs: [
            {
              url: "",
              title: "Page " + collection.state.currentPage.toString() + " of " + collection.state.lastPage
            }
          ]
        });
        return $("#breadcrumb").html(breadcrumb_template);
      },
      render: function() {
        var that;
        that = this;
        this.breadcrumb(paginatedPeople);
        this.$el.empty();
        if (!paginatedPeople) {
          ...
          if (paginatedPeople) {
            require(["backgrid", "backgrid-paginator"], function(Backgrid, BackgridPaginator) {
              ...
              paginator.listenTo(paginatedPeople, "reset", function() {
                router.navigate("list/page/" + this.collection.state.currentPage.toString(), {
                  trigger: false
                });
                that.breadcrumb(this.collection);
              });
            });
            this.$el.show();
          } else {
    ...
    router.on("route:home", function() {
      var peopleListView;
      app.paginatedPeople = new PaginatedPeople();
      peopleListView = new PeopleListView();
      app.paginatedPeople.getFirstPage({
        dataType: "jsonp"
      });
    });
    router.on("route:listPage", function(page) {
      var peopleListView;
      app.paginatedPeople = new PaginatedPeople();
      peopleListView = new PeopleListView();
